...
function login() {
    ...
    if (matchedUser) {
        ...
        if (document.getElementById('gstCheckbox')) {
            window.gstEnabled = document.getElementById('gstCheckbox').checked;
        }
...
let gstEnabled = true;  // NEW: GST toggle flag
...
// NEW: Recalculate total based on GST toggle
function recalcTotalAndDisplay() {
    ...
    if (discountApplied && window.discountPercent) {
        ...
        window.discountedGST = gstAmount;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // NEW: Attach GST toggle event listener
    const gstCheckbox = document.getElementById('gstCheckbox');
    if (gstCheckbox) {
...
async function downloadQuotation() {
    ...
    if (discountApplied) {
        ...
        // MODIFIED: Only apply GST on discount if gstEnabled
    ...
    doc.setFont("helvetica", "normal");
    // MODIFIED: Use correct amount for words based on GST toggle
    ...
    // Service Details & Deliverables Section (unchanged - too long but kept)
    ...
    // Terms & Conditions Section (kept as is - too long)
    doc.addPage();
    y = 20;
    addLogoToCurrentPage(doc);
...
function login() {
    ...
    if (matchedUser) {
        ...
        // Initialize GST checkbox
        const gstCheckbox = document.getElementById('gstCheckbox');
        if (gstCheckbox && typeof window.gstEnabled === 'undefined') {
            window.gstEnabled = true;
...
async function downloadQuotation() {
    ...
    // Terms & Conditions Section (Original full version)
    doc.addPage();
    y = 20;
    addLogoToCurrentPage(doc);
